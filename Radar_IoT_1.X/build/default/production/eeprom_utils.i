...
void iniValueEeprom(void) {
    ...
    temperatureLimit = DATAEE_ReadByte(2);
    if (temperatureLimit > 100) {
        temperatureLimit = 10;
        DATAEE_WriteByte(2, temperatureLimit);
        _delay((unsigned long)((20)*(64000000/4000.0)));
    }

    humidityLimit = DATAEE_ReadByte(1);
    if (humidityLimit > 100) {
        humidityLimit = 70;
        DATAEE_WriteByte(1, humidityLimit);
        _delay((unsigned long)((20)*(64000000/4000.0)));
    }

    fanCondition = DATAEE_ReadByte(0);
    if (fanCondition > 1) {
        fanCondition = 0;
        DATAEE_WriteByte(0, fanCondition);
        _delay((unsigned long)((20)*(64000000/4000.0)));
    }



    brightSlope = readEepromDoble(3);
    if (( __fpclassifyf(brightSlope) == 0 )) {
        writeEepromDouble(3, 0.0254);
        brightSlope = readEepromDoble(3);
    }

    brightOffset = readEepromDoble(7);
    if (( __fpclassifyf(brightOffset) == 0 )) {
        writeEepromDouble(7, -4.0821);
        brightOffset = readEepromDoble(7);
    }


    pwmSlope = readEepromDoble(11);
    ...
    speedBufferLen = DATAEE_ReadByte(19);
    ...
    speedBufferTimeout = DATAEE_ReadByte(20);
    if (speedBufferTimeout > 60 || speedBufferTimeout < 10) {
        speedBufferTimeout = 60;
    ...
    minimumBatVoltage = readEepromDoble(25);
    if (minimumBatVoltage < 9.0 || minimumBatVoltage > 16.0) {
        minimumBatVoltage = 10.5;
        writeEepromDouble(25, minimumBatVoltage);
    }
        __nop();

    batterySlope = readEepromDoble(30);
    if (batterySlope > 100.0 || batterySlope < 100.0) {
        batterySlope = 0.01;
        writeEepromDouble(30, batterySlope);
    }


    batteryOffset = readEepromDoble(34);
    if (batteryOffset > 100.0 || batteryOffset < 100.0) {
        batteryOffset = 0.0;
        writeEepromDouble(34, batteryOffset);
    }

    minimumSpeed = DATAEE_ReadByte(38);
    if (minimumSpeed > 20) {
        minimumSpeed=7;
