...
static int taskSetPwm(struct pt *pt) {
    ...
    while (1) {
        delay = millis() + 10000UL;
        do { (pt)->lc = 87; case 87:; if(!(millis() >= delay)) { return 0; } } while(0);
...
static int taskEvnHgm(struct pt *pt) {
    ...
    __nop();
    while (1) {
        delay = millis() + 600000UL;
        do { (pt)->lc = 116; case 116:; if(!(millis() >= delay)) { return 0; } } while(0);



        do { do { (pt)->lc = 120; case 120:; if(!((&mutexUart2Tx)->count > 0)) { return 0; } } while(0); --(&mutexUart2Tx)->count; } while(0);
        sendUart2Str("AT+GTDAT=gv300,0,,[ASR100|DS|1.0|EVT|HGR|");
        ...
        delay = millis() + 2000;
        do { (pt)->lc = 137; case 137:; if(!(millis() >= delay)) { return 0; } } while(0);
        ++(&mutexUart2Tx)->count;
    }
