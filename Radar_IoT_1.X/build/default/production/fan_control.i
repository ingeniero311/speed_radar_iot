...
static int taskCheckFan(struct pt *pt) {
    ...
    while (1) {
        ...
        delay = millis() + 30000;
        do { (pt)->lc = 35; case 35:; if(!(millis() >= delay)) { return 0; } } while(0);
        ...
        if (fanCondition == 1) {
            if (LATAbits.LATA4 == 0 && (currentTemperature < lowTemperatureLimit && currentHumidity > highHumidityLimit)) {
                do { do { (pt)->lc = 47; case 47:; if(!((&mutexUart2Tx)->count > 0)) { return 0; } } while(0); --(&mutexUart2Tx)->count; } while(0);
                sendUart2Str("AT+GTDAT=gv300,0,,[ASR100|DS|1.0|EVT|FON],0,,,,FFFF$\r\n");
                delay = millis() + 2000;
                do { (pt)->lc = 50; case 50:; if(!(millis() >= delay)) { return 0; } } while(0);
                ++(&mutexUart2Tx)->count;
                do { LATAbits.LATA4 = 1; } while(0);
            } else if (LATAbits.LATA4 == 1 && (currentTemperature > highTemperatureLimit || currentHumidity < lowHumidityLimit)) {
                do { do { (pt)->lc = 54; case 54:; if(!((&mutexUart2Tx)->count > 0)) { return 0; } } while(0); --(&mutexUart2Tx)->count; } while(0);
                sendUart2Str("AT+GTDAT=gv300,0,,[ASR100|DS|1.0|EVT|FOF],0,,,,FFFF$\r\n");
                delay = millis() + 2000;
                do { (pt)->lc = 57; case 57:; if(!(millis() >= delay)) { return 0; } } while(0);
                ++(&mutexUart2Tx)->count;
                do { LATAbits.LATA4 = 0; } while(0);
            }
        } else if (fanCondition == 0) {
            if (LATAbits.LATA4 == 0 && (currentTemperature < lowTemperatureLimit || currentHumidity > highHumidityLimit)) {
                do { do { (pt)->lc = 63; case 63:; if(!((&mutexUart2Tx)->count > 0)) { return 0; } } while(0); --(&mutexUart2Tx)->count; } while(0);
                sendUart2Str("AT+GTDAT=gv300,0,,[ASR100|DS|1.0|EVT|FON],0,,,,FFFF$\r\n");
                delay = millis() + 2000;
                do { (pt)->lc = 66; case 66:; if(!(millis() >= delay)) { return 0; } } while(0);
                ++(&mutexUart2Tx)->count;
                do { LATAbits.LATA4 = 1; } while(0);
            } else if (LATAbits.LATA4 == 1 && (currentTemperature > highTemperatureLimit && currentHumidity < lowHumidityLimit)) {
                do { do { (pt)->lc = 70; case 70:; if(!((&mutexUart2Tx)->count > 0)) { return 0; } } while(0); --(&mutexUart2Tx)->count; } while(0);
                sendUart2Str("AT+GTDAT=gv300,0,,[ASR100|DS|1.0|EVT|FOF],0,,,,FFFF$\r\n");
                delay = millis() + 2000;
                do { (pt)->lc = 73; case 73:; if(!(millis() >= delay)) { return 0; } } while(0);
                ++(&mutexUart2Tx)->count;
                do { LATAbits.LATA4 = 0; } while(0);
            }
        }
