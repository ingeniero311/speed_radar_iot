...
static unsigned int bufferAdcPanel[5];
static unsigned int bufferAdcBateria[5];
static unsigned int bufferAdcFoto[5];
static unsigned int bufferAdcHumedad[5];
...
static int taskAdcHum(struct pt *pt) {
    ...
    do { do { (pt)->lc = 40; case 40:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
    ADCC_StartConversion(ADC_HUM);
    do { (pt)->lc = 42; case 42:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
    ...
    while (1) {
        delay = millis() + 1000UL;
        do { (pt)->lc = 53; case 53:; if(!(millis() >= delay)) { return 0; } } while(0);

        do { do { (pt)->lc = 55; case 55:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
        ADCC_StartConversion(ADC_HUM);
        do { (pt)->lc = 57; case 57:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
        currentAdc = ADCC_GetConversionResult();
        ++(&mutexAdc)->count;
        ...
        bufferAdcHumedad[5 - 1] = currentAdc;
...
static int taskAdcMcp(struct pt *pt) {
    ...
    do { do { (pt)->lc = 96; case 96:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
    ADCC_StartConversion(ADC_MCP);
    do { (pt)->lc = 98; case 98:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
    ++(&mutexAdc)->count;
    currentAdc = ADCC_GetConversionResult();
    ...
    while (1) {
        delay = millis() + 1000UL;
        do { (pt)->lc = 108; case 108:; if(!(millis() >= delay)) { return 0; } } while(0);

        do { do { (pt)->lc = 110; case 110:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
        ADCC_StartConversion(ADC_MCP);
        do { (pt)->lc = 112; case 112:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
        currentAdc = ADCC_GetConversionResult();
        ++(&mutexAdc)->count;
        ...
        adcTemp = (summatory / 5)*(4.096 / 4096);

        currentTemperature = (adcTemp - (0.4)) / (0.01953);
        __nop();
...
static int taskAdcBat(struct pt *pt) {
    ...
    do { do { (pt)->lc = 151; case 151:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
    ADCC_StartConversion(ADC_BAT);
    do { (pt)->lc = 153; case 153:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
    currentAdc = ADCC_GetConversionResult();
    ++(&mutexAdc)->count;
    ...
    adcBattery = (summatory / 5);

    while (1) {

        delay = millis() + 1000UL;
        do { (pt)->lc = 171; case 171:; if(!(millis() >= delay)) { return 0; } } while(0);


        do { do { (pt)->lc = 174; case 174:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
        ADCC_StartConversion(ADC_BAT);
        do { (pt)->lc = 176; case 176:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
        currentAdc = ADCC_GetConversionResult();
        ++(&mutexAdc)->count;
        ...
        bufferAdcBateria[5 - 1] = currentAdc;
...
static int taskAdcFoto(struct pt *pt) {
    ...
    actdisplay = 0;
    delay = millis() + 5;
    do { (pt)->lc = 215; case 215:; if(!(millis() >= delay)) { return 0; } } while(0);

    do { do { (pt)->lc = 217; case 217:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
    ADCC_StartConversion(ADC_FOT);
    do { (pt)->lc = 219; case 219:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
    currentAdc = ADCC_GetConversionResult();
    ++(&mutexAdc)->count;
    ...
    adcVdcFoto = (summatory / 5);

    while (1) {

        delay = millis() + 10000UL;
        do { (pt)->lc = 238; case 238:; if(!(millis() >= delay)) { return 0; } } while(0);

        actdisplay = 0;
        delay = millis() + 5;
        do { (pt)->lc = 242; case 242:; if(!(millis() >= delay)) { return 0; } } while(0);


        do { do { (pt)->lc = 245; case 245:; if(!((&mutexAdc)->count > 0)) { return 0; } } while(0); --(&mutexAdc)->count; } while(0);
        ADCC_StartConversion(ADC_FOT);
        do { (pt)->lc = 247; case 247:; if(!(ADCC_IsConversionDone())) { return 0; } } while(0);
        currentAdc = ADCC_GetConversionResult();
        ++(&mutexAdc)->count;
        ...
        do { PT_YIELD_FLAG = 0; (pt)->lc = 255; case 255:; if(PT_YIELD_FLAG == 0) { return 1; } } while(0);

        for (i = 0; i < (5 - 1); i++) {
            bufferAdcFoto[i] = bufferAdcFoto[i + 1];
        }
        bufferAdcFoto[5 - 1] = currentAdc;
